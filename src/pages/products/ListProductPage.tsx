...
export function ListProductPage() {
  ...
  useEffect(() => {
    const products = JSON.parse(localStorage.getItem("products")!) as ProductDto[];
    if (products && products.length > 0) {
      dispatch(new ProductAction(productActions.PRODUCT_SUCCESS_PRODUCTS, products));
    } else {
      productService
        .getAllProducts()
        .then((data) => {
          productService.updateProducts$(data.products!);
          dispatch(new ProductAction(productActions.PRODUCT_SUCCESS_PRODUCTS, data.products!));
        })
        .catch((error) => {
